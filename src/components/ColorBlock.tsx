...
const ColorBlock = (props: ColorBlockTypes) => {
  ...
  return (
    <CopyToClipboard onCopy={handleCopy} text={props.color}>
      <div
        id={props.color}
        className={`w-full h-auto min-h-[60px]  flex flex-[1_1_200px] items-center justify-center cursor-pointer group ${props.isCurrentColor ? 'border-2 border-slate-200 dark:border-slate-950 outline-2 outline-black dark:outline-white outline' : ''}`}
